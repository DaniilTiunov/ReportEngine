...
from reportlab.platypus import (PageTemplate, SimpleDocTemplate, Paragraph, Spacer, Table, TableStyle, PageBreak,
...
def fillBodyPage(stand, doc, project):
    ...
    leftPartContent = [Paragraph(text="1. Основные сведения об изделии и и технические данные",
                                 style=titleStyle)]
    leftPartContent.append(Paragraph(
        text="1.1 Стенд датчиков КИПиА предназначены для установки на них манометрических и дифманометрических приборов, " +
        ...
        style=usualStyle))
    leftPartContent.append(
        Paragraph(text="1.2 Стенд датчиков имеет вид климатического исполнения и категорию УХЛ3.1 по ГОСТ-15150 и " +
                       "предназначен для эксплуатации в закрытых помещениях при температуре окружающего воздуха от плюс 5 до плюс 50 °С.",
                  style=usualStyle))
    leftPartContent.append(Paragraph(
        text="1.3 Стенд датчиков соответствует требованиям ТУ 4200-012-45633145-2016 и комплекту конструкторской документации.",
        style=usualStyle))
    leftPartContent.append(
        Paragraph(text="1.4 Изготовитель: ЗАО «ЭТАЛОН-ПРИБОР», 454138, г. Челябинск, пр. Победы, 288",
                  style=usualStyle))
    leftPartContent.append(Paragraph(
        text="1.5 Декларация соответствия требованиям ТР ТС 020/2011. Регистрационный номер ЕАЭС N RU Д-RU.PA01.B.94296/21. " +
             "Срок действия по 29.06.2024 включительно.",
        style=usualStyle))
    leftPartContent.append(
        Paragraph(text="1.6 Качество продукции обеспечено сертифицированной системой менеджмента качества, " +
                       "соответствующей требованиям ГОСТ ISO 9001-2011 (ISO 9001:2008).",
                  style=usualStyle))

    dimensions = (str(stand["Frames"][0]["Width"]), str(stand["Frames"][0]["Height"]),
                  str(stand["Frames"][0]["Depth"])) if len(stand["Frames"]) > 0 else ("____", "____", "____")
    ...
    leftPartContent.append(Paragraph(text="2.1 В комплект поставки входят:" + PdfHelper.newLineMark +
                                     ...
                                     style=usualStyle))
    leftPartContent.append(
        Paragraph(text="Примечание - датчики, манометры, вентильные блоки в комплект поставки стенда не входят.",
                  style=noteStyle))
    ...
    leftPartContent.append(Paragraph(text="3. Ресурсы, сроки службы, хранения и гарантии изготовителя",
                                     style=titleStyle))
    leftPartContent.append(Paragraph(
        text="3.1 Стенд датчиков в упаковке предприятия-изготовителя должен храниться в закрытых помещениях с естественной вентиляцией " +
        ...
        style=usualStyle))
    leftPartContent.append(Paragraph(
        text="3.2 Стенд датчиков в упаковке предприятия-изготовителя транспортируется любым видом крытого транспорта " +
             "с обеспечением защиты от механических повреждений. Условия транспортирования системы – С (средние) по ГОСТ-23170.",
        style=usualStyle))
    leftPartContent.append(
        Paragraph(text="3.3 Изготовитель гарантирует соответствие изделия требованиям ТУ 4200-012-45633145-2016 " +
                       "при соблюдении потребителем условий транспортирования, хранения, монтажа и эксплуатации",
                  style=usualStyle))
    leftPartContent.append(
        Paragraph(text="3.4 Гарантийный срок эксплуатации изделия – 24 месяца со дня ввода в эксплуатацию, " +
    ...
    productionInfoRowTable = Table(data=[[Paragraph(text="изготовлен", style=usualStyle),
                                          ...
                                          CreateSignatureTable("             ",
                                                               "№ опросного листа, № спецификации, РД")]])
    productionInfoRowTable.setStyle(TableStyle(cmds=PdfHelper.centerAlignTableStyleCmd +
                                                    [('VALIGN', (0, 0), (-1, -1), "TOP")]))

    productionSupervisorRowTable = Table(
        data=[[CreateSignatureTable("Начальник производственного участка", "Должность"),
    ...
    acceptanceSupervisorRowTable = Table(data=[[CreateSignatureTable("Представитель ОТК", "Должность"),
                                                CreateSignatureTable("                   ", "Подпись"),
                                                CreateSignatureTable(str(project["ResponsibleForAccept"]),
                                                                     "Расшифровка подписи")]])
    ...
    visualMeasuringSupervisorRowTable = Table(data=[[CreateSignatureTable("Представитель ОТК", "Должность"),
                                                     CreateSignatureTable("                   ", "Подпись"),
                                                     CreateSignatureTable(str(project["SecondLevelSpecialist"]),
                                                                          "Расшифровка подписи")]])
    ...
    rightPartContent.append(standInfoRowTable)
    rightPartContent.append(
        Paragraph(text="соответствует требованиям ТУ 4200-012-45633145-2016 и признан годным к эксплуатации.",
                  style=usualStyle))
    ...
    rightPartContent.append(standInfoRowTable)
    rightPartContent.append(
        Paragraph(text="контроль проведен в соответствии с ЭП-С.121.30.00, АКТ N 25-001", style=usualStyle))
    ...
    rightPartContent.append(standInfoRowTable)
    rightPartContent.append(
        Paragraph(text="упакован с соответствии с требованиями ТУ 4200-012-45633145-2016 и ГОСТ 23170-78.",
                  style=usualStyle))
