...
standTechCardHeaderTable = Table(
    data=[["Технологическая карта", PdfHelper.to_str(galvanizeStr), PdfHelper.to_str(project["RequestProduction"])]],
...
framePartsTable = Table(data=framePartsRecords,
                        colWidths=[leftPartWidth * 0.68, leftPartWidth * 0.12, leftPartWidth * 0.1,
                                   leftPartWidth * 0.1])
...
drainagePartsTable = Table(data=drainagePartsRecords,
                           colWidths=[rightPartWidth * 0.68, rightPartWidth * 0.12, rightPartWidth * 0.1,
                                      rightPartWidth * 0.1])
...
electricPartsHeaderTable = Table(data=[[Paragraph("Электрические компоненты", otherHeaderStyle)]],
                                 colWidths=rightPartWidth)
...
if imageExist:
    ...
    newWidth, newHeight, _ = PdfHelper.scaleImageToFit(blueprintOriginalWidth, blueprintOriginalHeight, rightPartWidth,
                                                       sumHeight)
...
electricPartsTable = Table(data=electricPartsRecords,
                           colWidths=[rightPartWidth * 0.68, rightPartWidth * 0.12, rightPartWidth * 0.1,
                                      rightPartWidth * 0.1])
...
def fillConclusionPage(stand, project):
    ...
    doneStagesTable = Table(data=doneTable,
                            colWidths=[sheetWidth * 0.05, sheetWidth * 0.2, sheetWidth * 0.1, sheetWidth * 0.125,
                                       sheetWidth * 0.125, sheetWidth * 0.1, sheetWidth * 0.15],
                            rowHeights=[sheetHeight * 0.08, sheetHeight * 0.05, sheetHeight * 0.05, sheetHeight * 0.05,
                                        sheetHeight * 0.05, sheetHeight * 0.05, sheetHeight * 0.05])
...
def generateReport(jsonFilePath, outputFilePath):
    ...
    for stand in data["Stands"]:
        standSheet = fillStandPage(stand, data,
                                   tableSplittingInfo=impulseTableInfo)  # теперь передаем инфу в заполнитель
