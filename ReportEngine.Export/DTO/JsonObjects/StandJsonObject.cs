...
public class StandJsonObject
{
    ...
    public List<FrameRecordJsonObject> Frames { get; set; } = new(); //рамы

    public List<PartRecordJsonObject> FrameParts { get; set; } = new(); //основные материалы рамы стенда

    public List<PartRecordJsonObject> MountParts { get; set; } = new(); //монтажные части

    public List<PartRecordJsonObject> DrainageParts { get; set; } = new(); //дренаж, продувка

    public List<PartRecordJsonObject> ElectricParts { get; set; } = new(); //электрические компоненты

    public List<ImpulseLineRecordJsonObject> ImpulseLines { get; set; } = new(); //импульсные линии в пределах стенда
}
