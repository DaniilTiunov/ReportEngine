...
public class CalculationService : ICalculationService
{
    ...
    private float? ObvAllTest(ProjectModel projectModel)
    {
        ...
        allTest = obvyazki.Count() *
                  _parametersStore[CalculationParameterType.HumanCost, "AllTestsTime"].Value.ToFloat();
    ...
    public float CalculatePaintTime(int frameCount, int obvCount, string isCabinet, bool isGalvanized)
    {
        ...
        var tcolr = frameCount * _parametersStore[CalculationParameterType.FrameCost, "FramePaintTime"].Value.ToFloat();
        // Время покраски всех обвязок
        var tcolob = obvCount *
                     _parametersStore[CalculationParameterType.FrameCost, "PipeworkPaintTime"].Value.ToFloat();
        // Tprep = Nr * время подготовки 1 рамы
        var tprep = frameCount * _parametersStore[CalculationParameterType.FrameCost, "FramePrepTime"].Value.ToFloat();
        // Tall = время подготовки всего оборудования
        var tall = _parametersStore[CalculationParameterType.StandCost, "EquipmentPrepTime"].Value.ToFloat();
        // Tsand = фиксированное время пескоструя
        var tsand = _parametersStore[CalculationParameterType.SandBlastCost, "SandblastingTime"].Value.ToFloat();
        // Итоговое время:
